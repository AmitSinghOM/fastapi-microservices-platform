...
| P2 | Phase 8 scripted clean-machine gate | Amended 2026-09-08: the 8/10 independent-developer study was descoped (solo project, no external testers). The gate is now a scripted fresh-container run of the adoption guide — install, authenticate, create org/project/key/endpoint, send a signed event, verify + durably accept in the receiver example, inspect the delivery — unattended within a 30-minute budget. Converts the blocker from a people problem into an engineering problem. | Agent can build the harness end-to-end. |
