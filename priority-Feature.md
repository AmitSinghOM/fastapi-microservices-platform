...
| P1 | Signed release candidate → real PyPI | **Phases A–E done 2026-09-10.** Signing configured and Verified; frozen candidate `198ba60` with green exact-SHA CI + Container; signed tag `sdk-v0.1.1`; TestPyPI run `34463988374` fully green (post-publish verification included); smoke install passed. `0.1.0` was burned on TestPyPI by a verifier bug (attestation sidecars) and superseded. **Remaining: Phase F** — production dispatch after the 24 h cooling-off (earliest 2026-09-11 ~15:30 IST), from the tag, reviewed from a second session. | Owner, ~10 min tomorrow. |
