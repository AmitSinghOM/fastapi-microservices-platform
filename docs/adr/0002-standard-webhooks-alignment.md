...
Introduce a per-endpoint, delivery-snapshotted **signature scheme** with
three values, and stage defaults across major versions. The envelope body,
event identity, retries, and all other wire behavior are unchanged.
...
- **`standard`** — spec-exact emission:
  - `webhook-signature: v1,<base64>` where the signed content is
    ...
    digest** for the endpoint's secret version.
  - During a signing-secret overlap window, sign with both the active and
    previous versions and send both signatures space-delimited, which
    upgrades rotation to the spec's zero-downtime model (legacy sends only
    one signature today).
  - `Webhook-Event` and `Webhook-Attempt` continue to be sent; the spec
    permits additive headers.
- **`dual`** — migration bridge: one header carrying both syntaxes,
  `t=<T>,v1=<hex> v1,<base64>` (legacy token first). Standard Webhooks
  libraries split on spaces and ignore the non-`v1` token; our documented
  legacy parsing rule likewise ignores the unknown token. Risk: a legacy
  receiver with a stricter parser than the documented contract may reject
  the whole header, which is why `dual` is opt-in per endpoint and never a
  default.
...
Switching an endpoint to `standard` or `dual` therefore behaves like a
secret rotation: the response returns the standard-form secret exactly once,
and the receiver installs it before or together with the scheme switch
(`dual` exists so this need not be atomic).
...
- Migration `0011`: `webhook_endpoints.signature_scheme` (`legacy|dual|standard`,
...
- `POST /v1/projects/{id}/endpoints` accepts optional
  ...
  returning the standard-form secret once when moving to `dual`/`standard`.
- SDK: `create_endpoint`/`update_endpoint` pass-through; the receiver module
  gains spec verification (`v1,<base64>` over `id.timestamp.body`) alongside
  legacy, selected by inspecting the header token shapes; `webhookctl`
  gains `--signature-scheme`.
...
| Release | Behavior |
| --- | --- |
| 3.x (next minor) | Feature ships complete but inert: everything defaults to `legacy`; `standard`/`dual` are per-endpoint opt-in. Wire bytes of existing endpoints are untouched, so this is a minor release. |
...
1. Golden test vectors for both schemes added to
   [the wire protocol](../wire-protocol.md), including a rotation-overlap
   header with two space-delimited signatures.
...
4. SDK receiver verifies both schemes and rejects a `dual` header whose two
   signatures disagree about the body.
...
- `dual` widens the legacy header with a token some untested receiver
  parsers may reject; this is contained by being per-endpoint opt-in.
